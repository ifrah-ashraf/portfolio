...
const projects = [
  ...
  },
  {
    ...
    tagline: "Context-aware retrieval pipeline",
    description:
    ...
    tech: ["Python", "FastAPI", "Qdrant", "Voyage AI", "Vector embeddings"],
    github: "http://github.com/ifrah-ashraf/rag/",
    live: null,
...
const colorMap: Record<
  string,
  { badge: string; border: string; glow: string; tag: string; check: string }
> = {
  cyan: {
    ...
    <section id="projects" className="w-full max-w-6xl mx-auto px-6 py-20">
      <div className="mb-12 text-center">
        ...
        <p className="text-white/40 mt-4 text-base max-w-xl mx-auto">
          Each project is a story of solving a real problem — with outcomes,
              ...
              {project.image && (
                <div className="relative w-full h-48 overflow-hidden border-b border-white/[0.05]">
                  ...
                  <div className="absolute top-3 left-3">
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full border backdrop-blur-sm ${colors.badge}`}
                    >
                      {project.status}
                    </span>
                  </div>
                  ...
                  <div className="flex-1">
                    {!project.image && (
                      <div className="flex items-center gap-3 mb-3">
                        <span
                          className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${colors.badge}`}
                        >
                          {project.status}
                        </span>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {project.tech.map((t) => (
                        <span
                          key={t}
                          className={`text-xs font-medium px-2.5 py-0.5 rounded-md border ${colors.tag}`}
                        >
                          {t}
                        </span>
                      ))}
                    ...
                    <div className="rounded-xl p-4 border border-white/[0.05] bg-white/[0.02]">
                      ...
                      <ul className="flex flex-col gap-2">
                        {project.outcomes.map((outcome) => (
                          ...
                          >
                            <svg
                              className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${colors.check}`}
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                              strokeWidth={2.5}
